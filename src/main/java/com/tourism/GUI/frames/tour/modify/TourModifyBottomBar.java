...
import com.tourism.GUI.frames.touristgroup.modify.TouristGroupBasicModifyPanel;
import com.tourism.GUI.util.ConfirmDialog;
import com.tourism.GUI.util.MessageDialog;

public class TourModifyBottomBar extends JPanel {
	...
	public void initComp() {
		btnCancel.addActionListener(new ActionListener() {	
			...
			public void actionPerformed(ActionEvent e) {
				TourMainPanel.initManagerPanel();
			}
		});
		...
		btnSave.addMouseListener(new MouseAdapter() {
			public void mousePressed(MouseEvent evt) {
				if(!TourBasicModifyPanel.commitToSelectedTouristGroup()) {
					new MessageDialog("Thông tin nhập không hợp lệ!");
					return;
				}
				tourController.saveWithRelationships(TourMainPanel.selectedTour);
				TourMainPanel.initManagerPanel();
			}
		});
			...
			public void mousePressed(MouseEvent evt) {
				if(TourMainPanel.selectedTour != null && TourMainPanel.selectedTour.getId() != null) {
					if(new ConfirmDialog("Xác nhận xóa").confirm())
						tourController.changeStatusToDeleted(TourMainPanel.selectedTour);
					TourMainPanel.initManagerPanel();
				}
			}
		});
